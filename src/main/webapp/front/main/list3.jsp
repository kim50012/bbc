...
var client = '';
 ...
 /* $(function(){
 	client = new ZeroClipboard($("#d_clip_button"));
	...
	//	  clipboard.setData( "text/plain",window.location.href);
 	...
 	client.on( 'ready', function(event) {
 	        //console.log( 'ready：movie is loaded' );
		...
		<ul>
			...
			<c:forEach var="goodsList" items="${goodsList}" varStatus="status">  
				<li >
					<div class="list-item">
						<div class="p">
							<a href="/front/detail/detail.htm?shopId=${goodsList.SHOP_ID}&displayId=${goodsList.DISPLAY_ID}&goodsId=${goodsList.GOODS_ID}" target="_detailPopupSkin5" title=""> <!-- target="_detailPopupSkin5"  -->
						...
						<div class="d">
							...
							<p class="d-price"><em class="h">￥${goodsList.PRICE_UNIT_SALES }</em><del onclick="javascript:window.open('/front/detail/detail.htm?shopId=${goodsList.SHOP_ID}&displayId=${goodsList.DISPLAY_ID}&goodsId=${goodsList.GOODS_ID}', 'detailPopup', '')"></del></p>
							<div class="d-main">
								<p class="d-freight">评分 : ${goodsList.REVIEW_POINT_AVG }分</p>
								<p class="d-num">
 ...
 <script id="tpl_list-search-result" type="text/html">
		{{each data}} 
				<li >
					<div class="list-item">
						<div class="p">
							<a href="/front/detail/detail.htm?shopId={{$value.SHOP_ID}}&displayId={{$value.DISPLAY_ID}}&goodsId={{$value.GOODS_ID}}" target="_detailPopupSkin5" title="">
