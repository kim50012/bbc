	...
	$('#foo').fadeIn(200);
	 ...
	 $.ajax({
	 	 		data:{
	 	 			pageCurr : pageCurr
	 	 			,pageUnit : pageUnit
	 	 			,blogTitle : $.trim($("#title").val())
	 	 			,blogGroupId : 0
	 	 			,shopGradeClassId : 0
	 	 			,orderby : "BLOG_ID"
	 	 		},
			type : "POST",
			url : "/front/main/blogGetListData.htm",
...
function fn_Search(newFlag, nextFlag) {
	...
	location.href =	'/front/blog/blogList.htm?pageCurr='+pageCurr
					+'&pageUnit='+pageUnit
					+'&orderby='+orderby
...
<body id="body" class="bodyColorBlogList">
...
<input type="hidden" id="hdblogGroupId" value="${blogGroupId }"/>
<input type="hidden" id="hdListClassName" value="${listClassName }"/>


<header class="headerList">
	<div class="searchPmd">
		<div class="title">
			<input id="title" type="text" value="${blogTitle }"/>	
		</div>
...
<div id="container"  style="margin-top: 0px;">
	...
	<div class="listMsg">
		<br>共有 <span id="spnTotalCount"> ${totalCount } </span> 个数据
	</div>
	...
	<div class="${listClassName }" id="J_SearchList">
		...
		<ul>
			...
			<c:forEach var="blogList" items="${blogList}" varStatus="status">  
				<li class="blogListLiCss" onclick="javascript:window.open('/front/blog/blogDetail.htm?shopId=${blogList.SHOP_ID}&blogId=${blogList.BLOG_ID}', 'detailPopup', '')">
					<div class="blogLiPadding">
						<div class="list-item-blog">
							<div class="p">
								<a href="/front/blog/blogDetail.htm?shopId=${blogList.SHOP_ID}&blogId=${blogList.BLOG_ID}" target="_blank" title=""> <!-- target="_blank"  -->
									<img class="p-pic" src="${blogList.BLOG_FILE_URL }" style="visibility: visible;">
									<span class="flag c-icon-pt" style="display:none;">${blogList.BLOG_GROUP_NM }</span>
								</a>
							</div>
							<div class="d">
								<a href="/front/blog/blogDetail.htm?shopId=${blogList.SHOP_ID}&blogId=${blogList.BLOG_ID}" target="_blank" title=""> <!-- target="_blank"  -->
									<h3 class="d-title">${blogList.BLOG_TITLE }</h3>
								</a>
								<p class="d-blgDetail">${blogList.BLOG_DETAIL }</p>
								<div class="d-main">
									<p class="d-freight">平分 : ${blogList.REVIEW_POINT_AVG }</p>
									<p class="d-num">
 ...
 <script id="tpl_list-search-result" type="text/html">
		{{each data}} 
				<li >
					<div class="list-item">
						<div class="p">
							<a href="/front/blog/blogDetail.htm?shopId={{$value.SHOP_ID}}&displayId={{$value.DISPLAY_ID}}&goodsId={{$value.GOODS_ID}}" target="_blank" title="">
								<img class="p-pic" src="{{$value.BLOG_FILE_URL}}" style="visibility: visible;">
								<span class="flag c-icon-pt" style="display:none;">{{$value.BLOG_GROUP_NM}}</span>
							</a>
						</div>
						<div class="d">
							<a href="/front/blog/blogDetail.htm?shopId={{$value.SHOP_ID}}&displayId={{$value.DISPLAY_ID}}&goodsId={{$value.GOODS_ID}}" target="_blank" title="">
								<h3 class="d-title">{{$value.BLOG_TITLE}}</h3>
							</a>
							<div class="d-main">
							...
							<span class="flag c-icon-p4p" onclick="javascript:window.open('/front/blog/blogDetail.htm?shopId={{$value.SHOP_ID}}&displayId={{$value.DISPLAY_ID}}&goodsId={{$value.GOODS_ID}}', 'detailPopup', '')">查看详情 >></span>
						</div>
					</div>
					<div class="icons-group"></div>
				</li>
