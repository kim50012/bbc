	...
	<body>
		<div class="content">
				...
				<div class="" style="height:0.7rem;width:100%;">
					<div class="float" style="width:100%;">
						<div class="list68 flex" style="padding:0;">
							<p class="flex1 switch-btn switch-btn1" onclick="window.location='clbEventList.htm?intClbsq=${amsClb.CLB_SQ}';"><span>진행중</span></p>
							<p><div class="parting-line"></div></p>
							<p class="flex1 switch-btn switch-btn2 active"><span>완료</span></p>
						</div>
					</div>
				</div>
			</div>
			
			<div class="switcharea-apply" style="display:none;">
			...
			<div class="switcharea-number">
				...
				<!-- START Data Loop -->
				<c:forEach var="atrEvtEndList" items="${atrEvtEndList}" varStatus="status">  
...
<%-- 												<img style="width:100%;height:auto;" src="${atrEvtEndList.PTO_URL}" onclick="eventImgView('${atrEvtEndList.PTO_URL}');"/> --%>
<!-- 											</div> -->
											<div class="flex1" style="padding-top:0.1rem;width:3.7rem;">
												<p class="lh03 word-title text-flow nowrap">${atrEvtEndList.HME_NM} vs ${atrEvtEndList.AWY_NM}</p>
												<p class="small22-text small22">${label.创建}${label.日期}：${atrEvtEndList.REG_DT}</p>
												<p class="small22-text small22 text-flow" style="white-space: normal">${atrEvtEndList.EVT_CTE}</p>
												<c:if test="${atrEvtEndList.VTC_TEM_DV == 'A'}">
													<p class="small22-text small22 text-flow" style="white-space: normal;color:red;">${atrEvtEndList.HME_NM} ${label.胜} !!</p>
												</c:if>
												<c:if test="${atrEvtEndList.VTC_TEM_DV == 'B'}">
													<p class="small22-text small22 text-flow" style="white-space: normal;color:red;">${atrEvtEndList.AWY_NM} ${label.胜} !!</p>
												</c:if>
											</div>
						</div>
						<div class="rate bg" style="padding-bottom:0.1rem;padding-top:0.1rem;">
							<div class="list68 flex rate-border" >
											<p class="flex1 switch-btn" style="text-align:center;line-height:1rem;"><span style="font-size:0.36rem;color: #f54141;" >${atrEvtEndList.A_TEM_SCR} </span><span style="font-size:0.18rem;">${atrEvtEndList.CNT_A} ${label.人参与}</span></p>
											<p><div class="parting-line" style="margin-top:0.34rem;"></div></p>
											<p class="flex1 switch-btn" style="text-align:center;line-height:1rem;"><span style="font-size:0.36rem;color: #f54141;">${atrEvtEndList.B_TEM_SCR} </span><span style="font-size:0.18rem;">${atrEvtEndList.CNT_B} ${label.人参与}</span></p>
							</div>
						</div>
						<div class="slid bg" style="padding-top:0.1rem;padding-bottom:0.2rem;">

								    <div class="open" style="border-top: 1px solid #ddd;">
	
								       <c:forEach var="atrEvtcheList" items="${atrEvtcheList}" varStatus="status">
								
								        <c:if test="${atrEvtEndList.MTC_SQ == atrEvtcheList.MTC_SQ}">
