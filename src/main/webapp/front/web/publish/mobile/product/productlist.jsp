...
<input type="hidden" id="hdShopGradeClassId" value="${shopGradeClassId }"/>
	...
	<div id="wrap">
		...
		<section id="container" class="brand-layout" style="min-height: 600px;">
			...
			<!-- 2015.10.29 brand-visual 위치 변경 -->
			<!--
			<div class="brand-visual"><img alt="" src="http://image.ssgdfm.com/images/shop/upload/059/83485YSL.jpg"></div>
			-->
			<div class="search-result">
				...
				<div class="search-menu">
					<ul>
			            <li <c:if test="${shopGradeClassId == 0 or shopGradeClassId == null}">class="on"</c:if>><a href="/front/web/publish/mobile/product/productlist.htm?shopGradeClassId=&orderby=${orderby }">전체상품</a></li>
						<c:forEach var="classList" items="${classList}" varStatus="status">
							<li <c:if test="${classList.SGC_ID == shopGradeClassId}">class="on"<c:set var="aTitle1" value="${classList.SGC_NM}" /></c:if>><a href='/front/web/publish/mobile/product/productlist.htm?shopGradeClassId=${classList.SGC_ID}&orderby=${orderby }'>${classList.SGC_NM}</a></li>
						</c:forEach>
					</ul>
				</div>
				...
				<div class="search-menu">
					<ul>
			            <li <c:if test="${brandId == 0 or brandId == null}">class="on"</c:if>><a href="/front/web/publish/mobile/brand/brand.htm?brandId=&shopGradeClassId=${shopGradeClassId}&orderby=${orderby }">전체</a></li>
				...
				<div class="search-menu">
					<ul>
						<li <c:if test="${orderby == ''}">class="on"<c:set var="aTitle3" value="최신상품" /></c:if>><a href="/front/web/publish/mobile/product/productlist.htm?brandId=${brandId}&shopGradeClassId=${shopGradeClassId}&orderby=">최신상품</a></li>
						<li <c:if test="${orderby == 'PRICE_ASC'}">class="on"<c:set var="aTitle3" value="가격 ↓" /></c:if>><a href="/front/web/publish/mobile/product/productlist.htm?brandId=${brandId}&shopGradeClassId=${shopGradeClassId}&orderby=PRICE_ASC">가격 ↓</a></li>
						<li <c:if test="${orderby == 'PRICE_DESC'}">class="on"<c:set var="aTitle3" value="가격 ↑" /></c:if>><a href="/front/web/publish/mobile/product/productlist.htm?brandId=${brandId}&shopGradeClassId=${shopGradeClassId}&orderby=PRICE_DESC">가격 ↑</a></li>
						<li <c:if test="${orderby == 'SALES_QTY_DESC'}">class="on"<c:set var="aTitle3" value="판매량" /></c:if>><a href="/front/web/publish/mobile/product/productlist.htm?brandId=${brandId}&shopGradeClassId=${shopGradeClassId}&orderby=SALES_QTY_DESC">판매량</a></li>
					</ul>
				</div>
							...
							<div class="detail-item">
								...
								<div class="detail-text">
									...
									<ul class="appraisal">
										...
										<li><strong>${goodsList.STOCK_QTY}</strong>개 평가</li>
									</ul>
	  ...
	  wx.config({
	  	...
	  	wx.ready(function(){
	  		wx.onMenuShareTimeline({
	  		    title: '상품목록 - 쇼핑몰 상품 목록 페이지 입니다.', // 分享标题
