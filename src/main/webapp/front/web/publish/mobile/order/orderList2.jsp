...
<title>我的订单</title>
<%@ include file="/front/skin/commonShop.jsp"%>
<%@ include file="/front/main/messageTip.jsp"%>
<style type="text/css">
	.sale_price .title_style {width: 65%;}
	.tabOrder li.on a, .tabOrder li a.on {
...
<form id="payform" action="" method="post">
	...
	<div class="myorder">
			...
			<ul class="tabOrder">
				<li id="tab1" style="width: 25%;" <c:if test="${orderIndex eq 1 }"> class="on" </c:if>><a href="orderList.htm">전체주문</a></li>
				<li id="tab2" style="width: 25%;" <c:if test="${orderIndex eq 2 }"> class="on" </c:if>><a href="orderList.htm?orderIndex=2">지불대기</a></li>
				<li id="tab3" style="width: 25%;" <c:if test="${orderIndex eq 3 }"> class="on" </c:if>><a href="orderList.htm?orderIndex=3">배송대기</a></li>
				<li id="tab4" style="width: 25%;" <c:if test="${orderIndex eq 4 }"> class="on" </c:if>><a href="orderList.htm?orderIndex=4">수령대기</a></li>
			</ul>
							...
							<c:when test="${orderIndex eq 2}">
				 				<div class="ui-noCount">
									<div class="ui-noCountImg">
								 	...
								 	<div class="ui-noCountText">조회된 주문이 없습니다.</div>
								 	<div class="ui-noCountBtn" style="margin: 20px auto;background-color: #da2028;" onclick="location.href='/front/web/publish/mobile/main/index.htm'">Home 가기</div>
								</div>
							</c:when>
						...
						<!-- ======================================== -->
							<c:if test="${list.FLAG eq 'Y' }">
								<div class="sale_price">
									...
									<span class="title_style"> &nbsp; 오더번호 : ${list.ORDER_OUT_ID }</span>   
