...
<script type="text/javascript">
	...
	function fnSetDefaultAddress(newDefalutId){
		...
		$.ajax({
			...
			type : "POST",
			url : "/front/customer/updateAddressDefault.htm",   //调用customer中的updateAdressDefault方法修改默认地址。
			success : function(data) {
				if(data.ret=="fail"){
					alert("修改默认地址失败");
				}else{
					//修改成功后，刷新当前页面，将默认地址，显示在最顶端。
					window.location.href = "/front/customer/myInformationUpdate.htm";
				}
			},
	...
	function updateCustAddress(addressId){
		//调用customer中的updateAddress（）方法跳转到地址修改页面。
		window.location.href = "/front/customer/updateAddress.htm"
							 + "?addressId=" + addressId;
	}
	//删除当前地址
	function deleteAddress(id){
		...
		if(window.confirm("确定删除此地址吗?")){
			$.ajax({
			 	data: { addressId : id },
				type : "POST",
				url : "/front/customer/deleteAddress.htm",  //调用customer中的deleteAddress方法，删除当前地址。
				success : function(data) {
					if(data.ret == "fail"){
						alert("删除地址失败!");
					}else{
						alert("删除地址成功!");
						//调用customer中的goodsReceipt方法，跳转到客户地址列表页面。
...
<body >
	...
	<%-- <form id="form1" action="/front/customer/orderConfirm.htm" method="post">
	...
	<div class="footerFixed">
		<div class="btnGroup agc">
			<a class="btn theme1" href="/front/orderConfirm/addressCreate.htm?shopId=${sessionSkin.shopId}">
