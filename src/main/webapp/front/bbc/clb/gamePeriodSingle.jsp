		...
		<div class="content">
			...
			<div class="modules" style>
				<div class="" style="height:0.7rem;width:100%;border-bottom:1px solid #ddd;">
					<div class="modules" style="background:#fff;padding-left:0.2rem;padding-right:0.2rem;">
						<div class="flex">
							<p class="flex1 title-icon">${label.期间别比赛现况}</p><!-- ${label.期间别比赛现况} -->
				...
				<div class="" style="height:0.7rem;width:100%;">
					<div class="month float" style="width:2rem;height:0.7rem;line-height:0.7rem;margin-right:0.1rem;">
						<input type="text" style="" name="datFrdt" id="datFrdt" value="${datFrdt }" placeholder="选择日期"><!-- ${datFrdt } -->
					</div>
			...
			<div class="module list-head" style="border-top:none;">
				<ul class="flex">
					...
					<li class="flex2 cell-6 cell" style="text-decoration:underline;" onclick="search('BBC');">득실</li>
				</ul>
			</div>
...
<!-- 				</ul> -->
			...
			function search(strOrderby) {
				window.location='gamePeriodSingle.htm?intClbsq=${intClbsq}&strOrderby='+strOrderby+'&datFrdt='+$("#datFrdt").val()+"&datTodt="+$("#datTodt").val();
			}
			function searchToday(intMbrsq) {
				window.location='gameSingleList.htm?intClbsq=${intClbsq}&intMbrsq='+intMbrsq+'&datFrdt='+$("#datFrdt").val()+"&datTodt="+$("#datTodt").val();
			}
     		$(function(){
     		 	$("#datFrdt").calendar({
