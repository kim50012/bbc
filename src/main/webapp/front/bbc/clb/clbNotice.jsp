	...
	<body>
		<div class="content">
			...
			<div class="switcharea-apply">
				...
				<c:set var="ntcTag" value="" />
				
				<!-- START Data Loop -->
				<c:forEach var="atrClbBbdList" items="${atrClbBbdList}" varStatus="status">
				  
					<c:if test="${atrClbBbdList.RCM_CNT > 0}">
						<c:set var="ntcTag" value="font-weight:bold;" />
					</c:if>
					<c:if test="${atrClbBbdList.RCM_CNT == 0}">
						<c:set var="ntcTag" value="" />
					</c:if>
...
<%-- 							<pre style="white-space: pre-wrap;">${atrClbBbdList.CLB_BBD_CTE}</pre> --%>
							<c:if test="${atrClbBbdList.REG_MBR_SQ == amsClbMbr.MBR_SQ}">
								<div class="flex">
									...
									&nbsp;
									<button id="btn" class="btn-join" onclick="window.location='/front/bbc/clb/clbNoticeReg.htm?intClbsq=${atrClbBbdList.CLB_SQ}&intClbbbdsq=${atrClbBbdList.CLB_BBD_SQ}';" style="right:0px;padding-right:0.2rem;">${label.修改}</button>
								</div>
							</c:if>
						</li>
					</ul>
