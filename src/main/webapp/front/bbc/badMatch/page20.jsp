...
// 						$("#mchDt").html(data.list[0].MCH_DT);
	...
	function getGameList(court, refreshFlag) {
		...
		load.show()
		 ...
		 $.ajax({
		 	 		data:{
		 	 			para1 : "BADMATCH_SELECT_COURT_GAME_LIST"
		 	 			,para2 : "${para3}"
		 	 			,para3 : court
		 	 		},
				type : "POST",
					...
					if (data.list.length != 0) {
						...
						for (var i = 0; i < data.list.length; i++) {
							...
							if (i%2 == 0) {
								...
								htm = ''
									+ '			<tr>'
									+ '				<td class="center"><b>['+gCnt+'경기]</b><br><span style="color:blue;">'+data.list[i].START_TM+'<span></td>'
									...
									+ '				<td class="center" onclick="gameResult('+data.list[i].MCH_REQ_A+', '+data.list[i].MCH_REQ_B+', '+data.list[i].FINAL_GAME_LAYOUT+', '+data.list[i].NEXT_SEQ+', '+scoreaa+', '+scorebb+');">'+tagBtn+'</td>'
									+ '			</tr>'
									;
								j++;
								gCnt++;
