...
<!-- 						</div> -->
						...
						<div class="list flex row1" style="padding-top:0.2rem;"><span class="text-outline">${label.场地}：</span><span class="blink" style="font-size: 0.3rem;">※</span>&nbsp;<p style="font-size: 0.3rem;color:red;">${amsExcList.EXC_TIT}</p>&nbsp;<span class="blink2" style="font-size: 0.3rem;">※</span></div>
						...
						<div class="flex Participant-title top-dashed">
							<p class="flex1 title-icon">
							${label.参加人员}
							<c:if test="${amsExcList.EXC_TP == '1003'}">
								(${amsExcList.JIN_EXC_CNT} / ${amsExcList.JIN_LES_CNT} / ${amsExcList.JIN_NO_CNT} / ${amsExcList.JIN_VT_CNT})
							</c:if>
							<c:if test="${amsExcList.EXC_TP != '1003'}">
								(${label.共}${amsExcList.JIN_EXC_CNT}+${amsExcList.JIN_LES_CNT}${label.个人}, ${label.取消} : ${amsExcList.JIN_NO_CNT})
							</c:if>	
							</p>
						</div>
						<div class="flex Participant-title" style="padding-top: 0;padding-bottom: 0.2rem;">		
							<p class="flex1" style="content: inherit;">&nbsp;</p>
							<button id="btn2" class="btn-join" style="position: absolute;margin-top: 0;background: #0093ff;">차량운행등록</button>
							<c:if test="${amsExcList.HME_CLB_SQ == 9}">
								<button id="btn3" class="btn-join" style="margin-top: 0;background: #0093ff;">콕구매신청</button>
							</c:if>
				...
				<div class="" style="height:0.7rem;width:100%;">
					<div class="float" style="width:100%;">
						<div class="list68 flex" style="padding:0;">
							<p class="flex1 switch-btn switch-btn1 active" ><span>참여순</span></p>
							<p><div class="parting-line"></div></p>
							<p class="flex1 switch-btn switch-btn2"><span>랭킹순</span></p>
							<p><div class="parting-line"></div></p>
							<p class="flex1 switch-btn switch-btn3"><span>댓글보기</span></p>
						</div>
					</div>
				</div>
			</div>
				...
				<div class="modules" style="border-bottom:none;">
					<div class="sub-content" id="subContentDiv">
						...
						<c:forEach var="amsJinList" items="${amsJinList}" varStatus="status">  
							...
							<c:if test="${amsExcList.MBR_SQ == amsJinList.MBR_SQ}">
								<c:set var="fontColor" value="color:#01cfb9;" />
							</c:if>
							<c:if test="${amsExcList.MBR_SQ != amsJinList.MBR_SQ}">
								<c:set var="fontColor" value="" />
							</c:if>
							<div class="list78 noborder bottom-dashed flex hang" style="padding:0;${excJinDeleteTag}">
								...
								<p class="flex1" style="max-width:75px;${fontColor }"><span class="${amsJinList.GND_DV_TAG}"></span>${amsJinList.MBR_NM}</p>
								<p class="member-head-wrap" style='${excJintpTag}'><img class="member-head" style='border-radius: 0;width:0.3rem;height:0.3rem;margin-top:0.23rem;' src="/front/bbc/img/em/${amsJinList.EM_TYPE}.gif"/></p>
								<p class="flex1" onclick="alert($(this).html());" style="${fontColor }">
								<c:if test="${amsJinList.EXC_JIN_TP == 7 and amsJinList.EXC_TP != '1003' and amsJinList.RANKING > 0}">[대기 ${amsJinList.EXC_JIN_CMT} 순위 ]</c:if>${status.count}.${amsJinList.EXC_JIN_CMT}
								</p>
								<p class="flex1 right" style="max-width:40px;${fontColor }" >${amsJinList.REGDT}</p>
							</div>
					...
					<div class="sub-content" id="subContentDiv">
						...
						<c:forEach var="amsJinList" items="${amsJinListRank}" varStatus="status">  
							...
							<div class="list78 noborder bottom-dashed flex hang" style="padding:0;${excJinDeleteTag}">
								<p class="member-head-wrap <c:if test="${amsJinList.MBR_SQ == loginMbrSq}"> del</c:if>"><img class="member-head" src="${amsJinList.MBR_MAI_IMG_PTH}"/></p>
								<p class="flex1" style="max-width:75px;${fontColor }" ><span class="${amsJinList.GND_DV_TAG}"></span>${amsJinList.MBR_NM}</p>
								<p class="member-head-wrap" style='${excJintpTag}${fontColor }' ><img class="member-head" style='border-radius: 0;width:0.3rem;height:0.3rem;margin-top:0.23rem;' src="/front/bbc/img/em/${amsJinList.EM_TYPE}.gif"/></p>
								<p class="flex1" onclick="alert($(this).html());" style="${fontColor }">
									${status.count}.${amsJinList.EXC_JIN_CMT}
								</p>
								<c:if test="${amsJinList.CLB_GD != 'E'}">
									<p class="flex1 right" style="max-width:40px;${fontColor }" >${amsJinList.CLB_RANK}${label.位 }</p>
								</c:if>
								<c:if test="${amsJinList.CLB_GD == 'E'}">
									<p class="flex1 right" style="max-width:40px;${fontColor }" ><img src="/front/bbc/image/c.jpg" style="height: 0.5rem;margin-left: -0.2rem;padding-top: 0.1rem;margin-right:0;width: 0.5rem;"></p>
								</c:if>
							</div>
			...
			<div class="switcharea-reply" style="display:none;">
		...
		<c:if test="${(amsExcList.CLB_GD == 'A') || amsClb.amsExcList == 'B'}">
			<c:if test="${(amsExcList.EXC_BBCOIN == 'N') and (amsExcList.HME_CLB_SQ == 23) and amsExcList.EXC_TP == '1001'}">
				<div style="text-align: center;margin-top: 0.2rem;">
						<button class="btn-submit" id="btnSave" onclick="fn_ExcJinBBCoin();" style="background-color: red;">참석자 BB Coin 주기</button>
				</div>
			</c:if>
		</c:if>
		
		<c:if test="${(amsExcList.CLB_GD == 'A') || amsClb.amsExcList == 'B'}">
			<c:if test="${(amsExcList.EXC_GAME == 'N') and ( amsExcList.EXC_TP == '1001' || amsExcList.EXC_TP == '1002') and amsExcList.JIN_EXC_CNT > 11}">
				<div style="text-align: center;margin-top: 0.2rem;">
		...
		<div class="pop-up-wrap" id="joinExc" style="display: none;">
			<div class="pop-up-content" style="padding-top:0.1rem;top:48%;">
				...
				<div class="buttons">
					...
					<span onclick="fn_ExcJinReg();">${label.参加}</span>
				</div>
			</div>
		</div>
