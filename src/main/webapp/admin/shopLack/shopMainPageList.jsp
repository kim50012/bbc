...
function fn_Main_Used(flag, pageId) {
	if(!confirm('<ui:i18n key="您确定设置该页面吗"/>?')){
		return;
	}
	if (flag == 0) { 
		 $.ajax({
	  	 		data:{
	  	 			pageId : pageId
	  	 			,jobType :'PAGE_ID_SHOP_MAIN'
	  	 		},
				type : 'POST',
				url : '/admin/shopLack/updateShopMainPageUse.htm',
				success : function(data) {
					alert('<ui:i18n key="保存成功"/>.');
					window.location.href = '/admin/shopLack/shopMainPageList.htm';
				},
...
function fn_Shop_Used(flag, pageId) {
	if(!confirm('<ui:i18n key="您确定设置该页面吗"/>?')){
		return;
	}
	if (flag == 0) {
...
function fn_MainPage_Delete(flag, pageId,mainFlag,homeFlag) {
	if(mainFlag == 1){
		alert('<ui:i18n key="该首页正在使用中无法删除"/>.');
		return;
	}
	if(homeFlag == 1){
		alert('<ui:i18n key="该店铺正在使用中无法删除"/>.');
		return;
	}
...
/* function fn_GetData(newFlag) {
	...
	if (newFlag) {
	 ...
	 $.ajax({
			...
			type : "POST",
			url : "/admin/shopLack/getShopMainPageList.htm",
			success : function(data) {
...
function query(pageNo, pageSize){
	...
	var url = '/admin/shopLack/shopMainPageFragment.htm';
	  			...
	  			<div class="result-grid" style="margin-top: 50px;">
			      	<div class="result-grid-header">
		   				<div class="result-grid-tools">
		   				   <span class="result-grid-tool" onclick="window.location='/admin/shopLack/addCustomPageEditor.htm'">
										...
										<table style="width: 92%;margin-left: 4%;padding: 0;">
											<tr style="border: 0">
												...
												<td style="width: 11%;padding: 0">
													<span class="gridbutton red"  onclick="window.location='/admin/shopLack/modifyCustomPageEditor.htm?pageId=${list.PAGE_ID}';"><ui:i18n key="编辑"/></span>
												</td>
												<td style="width: 26.5%;padding: 0">
													<c:choose>
														...
														<c:otherwise>
															<span class="gridbutton" style="color: #d7d7d7;" onclick="fn_Main_Used(${list.MAIN_FLAG}, '${list.PAGE_ID}');"><ui:i18n key="使用该首页"/></span>
														</c:otherwise>
													</c:choose>
												</td>
												<td style="width: 26.5%;padding: 0">
													<c:choose>
														...
														<c:otherwise>
															<span class="gridbutton" style="color: #d7d7d7;"  onclick="fn_Shop_Used(${list.HOME_FLAG}, '${list.PAGE_ID}');"><ui:i18n key="使用该店铺"/></span>
														</c:otherwise>
													</c:choose>
													
												</td>
												<td style="width: 11%;padding: 0">
													<span class="gridbutton red"  onclick="fn_MainPage_Delete(0, '${list.PAGE_ID}','${list.MAIN_FLAG}','${list.HOME_FLAG}');"><ui:i18n key="删除"/></span>
												</td>											
											</tr>
										</table>
									</td>
				...
				<td class="js-grid-gexing-nm">
					<table style="width: 92%;margin-left: 4%;padding: 0;">
						<tr style="border: 0">
							...
							<td style="width: 11%;padding: 0">
								<span class="gridbutton red"  onclick="window.location='/admin/shopLack/modifyCustomPageEditor.htm?pageId={{$value.PAGE_ID}}';"><ui:i18n key="编辑"/></span>
							</td>
							<td style="width: 26.5%;padding: 0">
								<c:choose>
									<c:when test="{{$value.MAIN_FLAG == '1' }}">
										<span class="gridbutton red" ><ui:i18n key="使用中"/></span>
									</c:when>
									<c:otherwise>
										<span class="gridbutton" style="color: #d7d7d7;" onclick="fn_Main_Used({{$value.MAIN_FLAG}}, '{{$value.PAGE_ID}}');"><ui:i18n key="使用该首页"/></span>
									</c:otherwise>
								</c:choose>
							</td>
							<td style="width: 26.5%;padding: 0">
								<c:choose>
									<c:when test="{{$value.HOME_FLAG == '1' }}">
										<span class="gridbutton red" ><ui:i18n key="使用中"/></span>
									</c:when>
									<c:otherwise>
										<span class="gridbutton" style="color: #d7d7d7;"  onclick="fn_Shop_Used({{$value.HOME_FLAG}}, '{{$value.PAGE_ID}}');"><ui:i18n key="使用该店铺"/></span>
									</c:otherwise>
								</c:choose>
								
							</td>
							<td style="width: 11%;padding: 0">
								<span class="gridbutton red"  onclick="fn_MainPage_Delete(0, '{{$value.PAGE_ID}}','{{$value.MAIN_FLAG}}','{{$value.HOME_FLAG}}');"><ui:i18n key="删除"/></span>
							</td>											
						</tr>
					</table>
				</td>
