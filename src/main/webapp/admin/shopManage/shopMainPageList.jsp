...
function fn_Main_Used(flag, pageId) {
	...
	if (flag == 0) { 
		 $.ajax({
				...
				type : 'POST',
				url : '/admin/shopManage/updateShopMainPageUse.htm',
	...
	if(confirm('<ui:i18n key="您确定要删除吗"/>?')) {
		if (flag == 0) {
			 ...
			 $.ajax({
					...
					type : 'POST',
					url : '/admin/shopManage/deleteDynamicPage.htm',
					success : function(data) {
...
/* function fn_GetData(newFlag) {
	...
	if (newFlag) {
	 ...
	 $.ajax({
			...
			type : "POST",
			url : "/admin/shopManage/getShopMainPageList.htm",
			success : function(data) {
...
function query(pageNo, pageSize){
	...
	var url = '/admin/shopManage/shopMainPageFragment.htm';
	  			...
	  			<div class="result-grid" style="margin-top: 50px;">
			      	<div class="result-grid-header">
		   				<div class="result-grid-tools">
		   				   <span class="result-grid-tool" onclick="window.location='/admin/shopManage/addCustomPageEditor.htm'">
				...
				<td class="js-grid-gexing-nm">
					<table style="width: 92%;margin-left: 4%;padding: 0;">
						<tr style="border: 0">
							...
							<td style="width: 11%;padding: 0">
								<span class="gridbutton red"  onclick="window.location='/admin/shopManage/modifyCustomPageEditor.htm?pageId={{$value.PAGE_ID}}';"><ui:i18n key="编辑"/></span>
							</td>
							<td style="width: 26.5%;padding: 0">
								<c:choose>
