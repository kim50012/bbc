...
$(function() {
	...
	$('.btn-step').click(function(){
		if($(this).find('span').text() == '<ui:i18n key="保存"/>'){
			...
			$.ajax({
				data:param,
				url : '/admin/shopManage/saveShopNavigation.htm', //调用AdminGoodsSellAction中updateGoodsGroup方法进行商品分类的修改。
				success : function(data){
					if(data.result == 'success'){
						alert('<ui:i18n key="保存成功"/>');	
						window.location.href = '/admin/shopManage/shopNavigation.htm';
					}else{
