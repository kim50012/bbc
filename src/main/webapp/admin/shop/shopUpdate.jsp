...
<title><ui:i18n key="店铺信息设置"/></title> 
<style type="text/css">
	.shop-set table{
		width:100%;
		position: relative;
...
function saveShop() {
    ...
    //获取公司名称
    var groupCorpName = $('#groupCorpName').find('option:selected').text();
    if(groupCorpName == ''){
    	alert('<ui:i18n key="请选择公司名称"/>');
    	return;
    }
    //获取公司ID
    var groupCorpId = $('#groupCorpName').find('option:selected').val();
    if(groupCorpId == ''){
    	alert('<ui:i18n key="请选择公司名称"/>');
    	return;
    }
    ...
    var imgLogoPicture1 = $('#logoFileId').val();
  //获取照片的文件名
    var logoFileName = '';
    if(imgLogoPicture1 == ''){
    	if(imgLogoPicture == ''){
        	...
        	return;
        	 ...
        	 if (type != 'jpg' && type != 'bmp' && type != 'gif' && type != 'png') {
            	 ...
            	 logoFileName = imgLogoPicture.substring(imgLogoPicture.lastIndexOf('\\') + 1, imgLogoPicture.length);
             } 
        }
    }
    imgLogoPicture1 = '${requestScope.adminShopFileMst.fileId }';
    //获取店铺简介
    var descIntroduction = $('#descIntroduction').val();
	if(descIntroduction == ''){
	...
	var param = {
			imgLogoPicture : imgLogoPicture1
    		,shopId : shopId
	...
	$.ajaxFileUpload ({
		data : param,
		url :'/admin/shop/updateShopBaseMessage.htm',   //调用AdminShopCreateAction类中的savefile()方法存储logo图片
		secureuri : false,
...
<div class="contents clearfix">
	...
	<div class="clearfix" style="margin-top: 40px;border-bottom:1px solid #eee;">
		<div class="title-warp">
			...
			<span class="title-name"><ui:i18n key="修改店铺信息"/></span>
		</div>
		...
		<table>
			...
			<tr>
				...
				<td style="padding-left: 20px; padding-top: 30px;">
					<input  type="text" name="shopName" id="shopName" disabled="disabled" value="${requestScope.adminShopEntity.shopName}" />
					<input type="hidden" id="shopId" value="${requestScope.adminShopEntity.shopId}" />
				</td>
			</tr>
			<!-- 选择店铺分类 -->
			<tr>
				...
				<td style="padding-left: 20px; position: relative;">
					<select  id="shopClassId" class="select2" data-placeholder="<ui:i18n key="请选择店铺分类"/>" onchange="selectCheck();">
						...
						<c:forEach items="${adminShopClassList}" var="listShopClass" varStatus="">
							<c:if test="${listShopClass.shopClassId==requestScope.adminShopEntity.shopClassId}">
								<option value="${listShopClass.shopClassId}" selected="selected">${listShopClass.shopClassName}</option>
							</c:if>
							<c:if test="${listShopClass.shopClassId!=requestScope.adminShopEntity.shopClassId}">
				...
				<td style="padding-left: 20px;position: relative;">
					<select  id="goodsClassId" class="select2" data-placeholder="<ui:i18n key="请选择主营产品"/>" onchange="selectCheck1();">
						...
						<c:forEach items="${adminGoodsClassList}" var="listGoodsClass" varStatus="">
							<c:if test="${listGoodsClass.goodsClassId==requestScope.adminShopEntity.mainGoodsClassId }">
								<option value="${listGoodsClass.goodsClassId}" selected="selected">${listGoodsClass.goodsClassName}</option>
							</c:if>
							<c:if test="${listGoodsClass.goodsClassId !=requestScope.adminShopEntity.mainGoodsClassId }">
			...
			<tr>
				...
				<div>
					<select  id="groupCorpName" class="select2" disabled="disabled"  data-placeholder="<ui:i18n key="请选择公司名称"/>" onchange="selectCheck2();">
						<option value=""> </option>
						<c:forEach items="${groupCorpList}" var="list" varStatus="">
							<c:if test="${list.groupCorpId==requestScope.adminShopEntity.groupCorpId }">
								<option value="${list.groupCorpId}" selected="selected">${list.groupCorpName}</option>
							</c:if>
							<c:if test="${list.groupCorpId!=requestScope.adminShopEntity.groupCorpId }">
								<option value="${list.groupCorpId}">${list.groupCorpName}</option>
							</c:if>
						</c:forEach>
					<select/>
					<!-- 显示检测信息 -->
			...
			<tr>
				...
				<td style="padding-left: 20px;">
					<input type="text" id="contactName" value="${requestScope.adminShopEntity.contactName }" placeholder="<ui:i18n key="请填写联系人姓名"/>"/>
					<!-- 显示检测信息 -->
			...
			<tr>
				...
				<td style="padding-left: 20px;">
					<input type="text" id="contactMobileNo" value="${requestScope.adminShopEntity.contactMobileNo }"  placeholder="<ui:i18n key="请填写联系人手机号"/>"  />
					<!-- 显示检测信息 -->
			...
			<tr>
				...
				<td style="padding-left: 20px;">
					<input type="text" id="contactEmail"  value="${requestScope.adminShopEntity.contactEmail }"  placeholder="<ui:i18n key="请填写联系人邮箱"/>"/>
					<!-- 显示检测信息 -->
			...
			<tr>
				...
				<td style="padding-left: 20px;">
					<input type="text" id="contactQQ" value="${requestScope.adminShopEntity.contactQQ }" placeholder="<ui:i18n key="请输入QQ号"/>"/>
					<!-- 显示检测信息 -->
			...
			<tr>
				...
				<td style="padding-left: 20px;padding-top:10px;padding-bottom:20px;height:100px;">
					...
					<div class="img-show-before" id="preview">
						...
						<img src="${requestScope.adminShopFileMst.fullURL }" alt="" id="logoUploadImg"/>
					</div>
					<span class="img-width-span" ><ui:i18n key="图片推荐尺寸"/>:(60px * 60px)</span>
					<input type="hidden" value="${requestScope.adminShopFileMst.fileId }" id="logoFileId"/>
			...
			<tr>
				...
				<textarea rows="4" cols="50" placeholder="<ui:i18n key="请输入店铺简介"/>" id="descIntroduction">${requestScope.adminShopEntity.descIntroduction }</textarea>
					<!-- 显示检测信息 -->
