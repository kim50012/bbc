...
<script type="text/javascript">
	...
	function gets_value(){
		...
		window.location.href = '/admin/sendMsgAll/selectMusic.htm?fileGroup=' + ${fileGroup} + '&pageCurr='+pageCurr;
	}
	...
	function selectByName(){
		...
		window.location.href = '/admin/sendMsgAll/selectMusic.htm?fileGroup=' + ${fileGroup} + '&fileName='+fileName;
	}
	//上传文件
	function publishFile(file){
	    ...
	    if(type != 'amr' && type != 'mp3'){
	    	 alert('<ui:i18n key="音频格式不正确"/>');
	    	 return;
	    }
	    var fileSize = file.files[0].size;
	    if(fileSize > (2*1024*1024)){
	  		alert('<ui:i18n key="音频太大"/>,<ui:i18n key="请重新选择"/>');
	  		return;
	  	}
		var param = { 
	...
	<div class="container">
		<div class="title-out-div">
			<div class="title-left-div">
				<span class="upload-file-img"><ui:i18n key="上传本地音频"/></span>
				      		 	...
				      		 	<c:forEach items="${list}" var="list" varStatus="status">
				      		 		<tr>
					      		 		...
					      		 		<td  class="title_show_td">
											<span style="margin-left:10px;" title="${list.FILE_NM }">${list.FILE_NM }</span>
										</td>
