...
<title><ui:i18n key="素材管理"/>::<ui:i18n key="修改多图文素材"/></title> 
...
$(function(){
	...
	$('#saveMaterial').click(function(){ 
		...
		pp.newslist = newslist;
		
		$('body').showLoading({'vPos':'200px'   });
		$.ajax({
			url : "/admin/weixin/material/modifyNews.htm",
			type : "POST",
			data : {
	...
	//预览
	$('#sendPreviewNews').click(function(){ 
		...
		var openid = '';
		//选中
		if($('.js-useritem.selected').size() != 1){
			alert('<ui:i18n key="请选择用户"/>!');
			return false;
		}
		$('.js-useritem.selected').each(function(){
		...
		$('#bpoupUser').showLoading({'vPos':'200px'   });
		$.ajax({
			url : "/admin/weixin/material/previewNews.htm",
			type : "POST",
			data : {
				jsonStr : JSON.stringify(pp)
			},
			success : function(data) {
				$('#bpoupUser').hideLoading();
				if (data.success) {
					alert('<ui:i18n key="发送成功"/>');
					window.location.reload();
				} else {
					alert('<ui:i18n key="发送失败"/>!' + data.msg );
				}
			},
			error : function(xhr, status, e) {
				$('#bpoupUser').hideLoading();
	...
	$('#closeUserWindow').click(function(){
		closeUserWindow();
	});
	
	
	//选中
	$('.js-useritem').click(function(){
		$('.useritem').removeClass('selected');
		$(this).addClass('selected');
	});
	
	//取消选中
	$('.js-useritem .user_mask').click(function(event){ 
		$(this).closest('.useritem').removeClass('selected');
		event.stopPropagation();    //  阻止事件冒泡
	});
	//添加用户
	$(".add-cust-li").click(function(){
		/* var $win = $('#bpoupCustSelect');
		...
		$win.bPopup({
		   	...
		   	position: [($(window).width() -$win.width()) /2, ($(window).height() -$win.height()) /2]
		 }); */
		showIframeBpopupWindow({
			id : 'bpoupMessageCust',
			width : 670,
			height :540,
			action : $('#iframe_cust').attr('src')
		}); 
	});
	...
	$('.img-show-able').hover(function(){
		if($(this).attr('class') == 'img-show-able img-selected'){
			$(this).parent().find('.delete-img').show();			
		}
	},function(){
		$(this).parent().find('.delete-img').hide();
	});
});
//wangbin modify
...
function closebpoupCust(){
	closeBpopupWindow('bpoupMessageCust');
}
//弹出窗口
...
function closeCustDiv(){
	$('#bpoupCustSelect').bPopup().close();
}
function addCustList(){
	closebpoupCust();
	$.ajax({
		url : "/admin/weixin/material/getCustWxList.htm",
		type : "POST",
		data : {},
		success : function(data) {
			var list=data.list;
			if(list.length>0){
				$("#addCustList").children().remove();
				var htm='<li class="add-cust-li" style="cursor: pointer;"> <img src="/admin/images/sendMsgAll/add.png" />'+
		 				'<h3 class="nickname"><ui:i18n key="添加用户"/></h3> </li>';
		 		$("#addCustList").append(htm);
		 		//添加用户
		 		$(".add-cust-li").click(function(){
		 			/* var $win = $('#bpoupCustSelect');
				...
				for(var i=0;i<list.length;i++){
						var html='<li data-openid="'+list[i].OPENID+'" class="useritem js-useritem" >'+
	 		 			'<img alt="" src="'+list[i].HEADIMGURL+'" class="headimgurl" ><h3 class="nickname">'+list[i].NICKNAME
	 		 			+'</h3><div class="user_mask"> <span class="maskicon"> </span> </div> </li>';
						$("#addCustList").append(html);
				}
				//选中
				$('.js-useritem').click(function(){
					$('.useritem').removeClass('selected');
	...
	$.ajax({
		...
		<div class="ui-right-content app-design clearfix" >
										...
										<div  class="media_preview_area"  >
											 <div class="appmsg multi editing">
												<div id="js_appmsg_preview" class="appmsg_content">
													...
													<c:forEach items="${requestScope.newsList}" var="item" varStatus="status">
														<c:choose>
															<c:when test="${status.index eq 0}">
																<div id="appmsgItem${status.index}" data-fileid data-id="${status.index}"  data-cover-type="big" 
																	data-news-id="${item.NEWS_ID }"
																	data-news-type="${item.NEWS_TYPE }"
																	data-title="${item.MTITLE }" 
																	data-author="${item.MAUTHOR }"
																	data-author="${item.MAUTHOR }"
																	data-content-source-url="${item.MSOURCE_URL }"
																	data-digest="${item.MDIGEST }"
																	<%-- data-content='${item.MCONTENT }' --%>
																	data-content="<c:out value="${item.MCONTENT }"></c:out>"
																	data-img-url="${item.FILE_URL }"
																	data-file-id="${item.MFILE_ID }"
																	data-show-cover-pic="${item.MSHOW_COVER_PIC }"
																	data-list-img-url="${item.LIST_IMG_URL }"
																	data-list-img-id="${item.LIST_IMG }"
															...
															<c:otherwise>
																<div id="appmsgItem${status.index}" data-fileid data-id="${status.index}"  data-cover-type="small" 
																	data-news-id="${item.NEWS_ID }"
																	data-news-type="${item.NEWS_TYPE }"
																	data-title="${item.MTITLE }" 
																	data-author="${item.MAUTHOR }"
												 ...
												 <a onclick="return false;" style="display: none;" class="create_access_primary appmsg_add" id="js_add_appmsg" href="javascript:void(0);">
												 	<ui:i18n key="添加"/>
												 </a>  
												  ...
												  <span class="title-span" style="margin-top:15px;"><ui:i18n key="封面"/><span style="color:#d1102d;">*</span>：</span>  
													<div class="img-preview" style="padding-top:10px;">
														<!--  <form  id="fileupload" action="" method="POST" enctype="multipart/form-data">
														 ...
														 <span class="jsCoverImgTig" style="color:#d1102d;font-size:12px;"></span>
													</div>
											</div>
											<%-- <div  style="width:100%;">
				...
				<div class="btnArea" >
					...
					<span id="previewMaterial" class="button bg-red" style=""><ui:i18n key="预览"/></span> 
				</div>
		</div>
		
	</div>
	<!-- ▲ Main container -->
		<div class="bpopup" id="bpoupUser" style=" height:470px; width:800px; ">
		 	<div class="bpopupWarp">
		 		 <div class="bpopupHeaderWarp" style="background-color: #636363;">
					<h3 class="bpopupTitle js-title"><ui:i18n key="选择接收者"/></h3>
					<div class="bpopupColseWarp">
						<img class="b-close" src="/admin/images/goodsSell/close.png" />
					</div>
				</div>
		 		 <div class="bpopupContentWarp" style=" height:350px;overflow-y:scroll;"> 
		 		 	<div class="userlist" >
		 		 	<ul id="addCustList">
		 		 		<li class="add-cust-li" style="cursor: pointer;">
		 		 			<img src="/admin/images/sendMsgAll/add.png" />
		 		 			<h3 class="nickname"><ui:i18n key="添加用户"/></h3>
		 		 		</li>
			 		 	 <c:forEach items="${requestScope.userList}" var="item" varStatus="status">
			 	 			<li data-openid="${item.OPENID }" class="useritem js-useritem" >
			 		 			<img alt="" src="${item.HEADIMGURL }" class="headimgurl" >
		 		 ...
		 		 <div style="position: absolute;bottom: 0; left: 0;top:410px;width: 100%;">
		 		 	
						<div class="btnArea" >
							<span id="sendPreviewNews" class="button bg-red" style="margin-top:5px;"><ui:i18n key="预览"/></span>
							<span id="closeUserWindow" class="button bg-red" style="margin-top:5px;"><ui:i18n key="关闭"/></span>
						</div>
		 		 </div>
		 	</div>
		</div>
		...
		<%-- <div class="bpopup" id="bpoupAlbum">
		 	<div class="bpopupWarp">
		 		 <div class="bpopupHeaderWarp">
					<h3 class="bpopupTitle js-title"><ui:i18n key="选择素材"/></h3>
					<div class="bpopupColseWarp">
						<img class="b-close" src="/admin/images/goodsSell/close.png" />
					</div>
				</div>
		 		 <div class="bpopupContentWarp"> 
		 		 	<iframe class="js-content-iframe"  width="100%" height="100%" frameBorder=0 scrolling=yes  src="/admin/weixin/material/showImageWindow.htm?fileGroup=2&type=2"></iframe>
		 		 </div>
		 	</div>
		</div> --%>
		<!-- ▲ bpopup area -->
	<%-- 	<div class="bpopup bpoupCustSelect" id="bpoupCustSelect">
		 	<div class="bpopupWarp" style="width:950px;">
		 		 <div class="bpopupHeaderWarp">
					<h3 class="bpopupTitle js-title"><ui:i18n key="选择客户"/></h3>
					<div class="bpopupColseWarp">
						<img class="b-close" src="/admin/images/goodsSell/close.png" />
					</div>
				</div>
		 		 <div class="bpopupContentWarp"> 
		 		 	<iframe   width="100%" height="100%" frameBorder=0 scrolling=yes  src="/admin/weixin/material/selectCust.htm"></iframe>
		 		 </div>
		 	</div>
		</div> --%>
		 <!-- ▼ bpopup area -->
				<div class="bpopup2" id="bpoupMessageCust" >
				 	<div class="bpopupWarp">
				 		 <div class="bpopupHeaderWarp">
							<h3 class="bpopupTitle js-title"><ui:i18n key="选择客户"/></h3>
							<div class="bpopupColseWarp">
				 		 ...
				 		 <div class="bpopupContentWarp"> 
				 		 	<iframe class="js-content-iframe"  width=100%; height=100%; id="iframe_cust"  style="overflow: hidden;" src="/admin/weixin/material/selectCust.htm"></iframe>
				 		 </div>
				 	</div>
				</div>
			<!-- ▲ bpopup area -->
