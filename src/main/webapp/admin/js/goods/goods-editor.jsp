...
function render07(){
	 console.log('====render07()====='); 
	 
	  //var id				=  $('.app-sidebar').attr('data-id');
	...
	//console.error('render07() tpl=\n' +tpl);
	 $('.editing').after(tpl).remove();
	 $('.app-field').unbind('click').click(bindShowSidebar);
	 
	 $('.app-field .actions .action.delete').unbind('click').click(function(){
...
function render52(){
	 console.log('====render52()=daikaifa===='); 
	  ...
	  console.log('size=' + size + ',itme.length=' + item.length);
	  
	  
	 
	 $('.editing').attr('data-field-item',JSON.stringify(item));
	 $('.editing').attr('data-field-size', size);
	 ...
	 var tpl = _.template($("#tpl_type_52_u").html());
	 var tpl2 =  tpl(paramObj);
	 
  	// console.error('render52() tpl2=\n' +tpl2);
 
     $('.editing').find('.control-group').html(tpl2);
	 $('.app-field').unbind('click').click(bindShowSidebar);
...
//在2张图(type-13)编辑器的 app-sidebar中删除图片触发事件
 ...
 //在2张图(type-18)编辑器的 app-sidebar中删除图片触发事件
   ...
   //在 (type-52)的 app-sidebar中删除图片触发事件
   function fnSidebar52ImgRemove(){
...
/***********************************************************/
 ...
 //显示编辑器功能
   function bindShowSidebar(){  
   	...
   	//console.log('bindShowSidebar id=' + id +' , type=' + type);
    	if(type == 'detail'){
    		return ;
    	}
        ...
        data.type = type;
     	if(type == 'header'){
    	   ...
    	   $('.js-add-goods').click(function(){
    		   showBpopupWindow({
    			   id:'bpopup',
    			   action: '/admin/weixin/material/showImageWindow.htm?fileGroup=1&type=3'
    		   }); 
    	   });
     		...
     		$('.app-sidebar').html(template('tpl_sidebar_01', data));
   		  	...
   		  	$('.js-add-goods').click(function(){
   		  		...
   		  		showBpopupWindow({
   		  			id:'bpopup',
   		  			action: '/admin/weixin/material/showImageWindow.htm?fileGroup=1&type=4'
   		  		}); 
    	    ...
    	    data.item = JSON.parse(item);
   			...
   			$('.app-sidebar').html(template('tpl_sidebar_52', data));
   		
   			$('.form-horizontal input:radio,.form-horizontal input:checkbox').click(render52);
   			...
   			$('.js-add-goods').click(function(){
   	   		  ...
   	   		  $('.js-img-area a.close-modal').unbind('click').click(fnSidebar52ImgRemove);
   	   ...
   	   data.item = JSON.parse(item);
   		$('.app-sidebar').html(template('tpl_sidebar_12', data));
   		  ...
   		  $('.js-add-goods').click(function(){
   			  ...
   			  showBpopupWindow({
		  			id:'bpopup',
		  			action: '/admin/weixin/material/showImageWindow.htm?fileGroup=1&type=5'
		  		});
   			   return false;
   			  	/*$('#modal_12').modal({
   				  	backdrop: 'static', 
   		  ...
   		  $('.js-add-goods').click(function(){
   			...
   			showBpopupWindow({
	  			id:'bpopup',
	  			action: '/admin/weixin/material/showImageWindow.htm?fileGroup=1&type=6'
	  		});
			 /*  return false;
   	   ...
   	   var item = $this.attr('data-field-item') || '[]';
   		...
   		$('.app-sidebar').html(template('tpl_sidebar_18', data));
   		  ...
   		  $('.js-add-goods').click(function(){
   			...
   			showBpopupWindow({
	  			id:'bpopup',
	  			action: '/admin/weixin/material/showImageWindow.htm?fileGroup=1&type=7'
	  		});
			   return false;
...
function setttingType52Image(imageArray){
	...
	var size = $('.editing').attr('data-field-size');
	if(size == 0){
		array = _.last(array, 3);
	}else{
		array = _.last(array, 4);
	}
	$editing.attr('data-field-item',JSON.stringify(array));
	...
	for(var i = 0 ; i < array.length;i++){
		array[i].idx = i + 1;
		var tpl =  template('tpl-modal_goods_item_52', array[i]);
		$('.js-img-area').append(tpl);
	}
	...
	$('.js-img-area li.sort').hover(function(){
  	...
  	$('.js-img-area a.close-modal').unbind('click').click(function(){
  		 alert();
  	}/* fnSidebar3ImgRemove */);
