...
.imgFile.exc.publish{
	background-image: url("/admin/images/myFile/exc_publish.png");
}
.imgFile.exc.none{
	background-image: url("/admin/images/myFile/exc_none.png");
}
...
$(function(){
	...
	$('.imgFile').click(function(){
		...
		$('.imgFile.exc').removeClass('publish').addClass('none');
		...
		if(item =='4'){
			$('.publishRule span').text('<ui:i18n key="视频最大支持10MB的视频"/>(xls/xlsx)');
			$('#fileUrlShow').val('');
		}
...
//删除文件
function deleteShopFile(fileId){
	var flagDelete = window.confirm('<ui:i18n key="您确定删除该文件吗"/>？');
	if(!flagDelete){
		return;
	}
	...
	$.ajax({
		data : param,
		url : '/admin/weixin/material/deleteShopFile.htm', //调用AdminGoodsSellAction中updateGoodsGroup方法进行商品分类的修改。
...
function query(pageNo, pageSize){
	...
	//window.location.href = '/admin/weixin/material/materialManage.htm?fileType='+fileType+'&fileName='+fileName+'&beginDate='+dateBegin+'&endDate='+dateEnd+'&pageCurr='+pageNo+'&pageUnit='+pageSize;
	 var url = '/admin/weixin/material/materialManageListFragment.htm';
...
function updateShopFile(fileId){
	...
	$.ajax({
		...
		type : 'GET',
		cache : false,
		url : '/admin/weixin/material/updateShopFile.htm', //调用AdminGoodsSellAction中updateGoodsGroup方法进行商品分类的修改。
...
function fileShowUrl(file){
    ...
    if(fileType == '1' && type != 'jpg' && type != 'bmp' && type != 'gif' && type != 'png'){
    	 ...
    	 return;
	   	 ...
	   	 $('#fileUrlShow').val('');
	   	 return;
    }else  if(fileType == '4' && type != 'xlsx' && type != 'xls'){
      	 alert('<ui:i18n key="视频格式不正确"/>');
      	 $('#fileUrlShow').val('');
      	 return;
   }
		...
		<div class="query-from">
       		<div class="form-table" >
       		 	<table class="ftable">
       		 		...
       		 		<tr>
       		 			<td class="flabel"><ui:i18n key="文件类型"/>：</td>
       		 			<td class="fcomp">
								...
								<option value="1" <c:if test="${requestScope.fileType==1 }"> selected="selected"</c:if>><ui:i18n key="图片"/></option>
								<option value="2" <c:if test="${requestScope.fileType==2 }"> selected="selected"</c:if>><ui:i18n key="音乐"/></option>
								<option value="3" <c:if test="${requestScope.fileType==3 }"> selected="selected"</c:if>><ui:i18n key="视频"/></option>
								<option value="4" <c:if test="${requestScope.fileType==4 }"> selected="selected"</c:if>>EXCEL</option>
							</select>
							<span style="width:80px;display: inline-block;"><ui:i18n key="文件名"/>：</span> 
					...
					<tr>
       		 			...
       		 			<td class="fcomp dateSelect">
       		 				<input type="text" class="dateBegin datepicker calendar" style="margin-left:-20px;" value="${requestScope.beginDate}" /> 
      	...
      	<div class="result-grid" style="margin-top:50px;">
	      	<div class="result-grid-header">
				<div class="result-grid-tools">
					<span class="result-grid-tool" onclick="createMaterial();">
			      		 	...
			      		 	<c:forEach items="${list}" var="list" varStatus="status">
			           			<tr>
			           				<td class="pic">
										...
										<c:if test="${list.FILE_TYPE ==4}">
											<a href="${list.FULL_URL }">${list.FILE_ID}</a>
										</c:if> 
			           				</td>
			           				<td>
			           					<div ></div>
			 		 	...
			 		 	<table class="poupMessageTable">
							<tr class="tableSpanMessage">
								<td>
									...
									<span data-item='4' class="imgFile exc publish"></span>
