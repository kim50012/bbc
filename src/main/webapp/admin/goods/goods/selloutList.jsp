...
<script type="text/javascript">
	...
	function query(pageNo, pageSize){
		...
		var url = '/admin/goods/goods/selloutListFragment.htm';
	...
	function bPoupDivDisplay(displayId){
		if(displayId == 'list'){
			...
			$('input[name="displayId"]:checked').each(function(){  
				...
				if($("#isDisplay" + $(this).val()).val() == '0'){
					flag=false;
				}
			});
			if(!flag){
				alert('<ui:i18n key="选中了不可下架的商品"/>，<ui:i18n key="请重新选择"/>');
				return;
			}
			if(display == ''){
	...
	//
	function bPoupDivDisplayPublish(displayId){
		if(displayId == 'list'){
			var display = "";  
			var flag=true;
			$('input[name="displayId"]:checked').each(function(){  
				display = display + ',' + $(this).val();//向数组中添加元素  
				if($("#isDisplay" + $(this).val()).val() == '1'){
					flag = false;
				}
			});
			if(!flag){
				alert('<ui:i18n key="选中了不可上架的商品"/>，<ui:i18n key="请重新选择"/>');
				return;
			}
			if(display == ''){
		...
		<div class="result-grid pic" style="margin-top: 30px;">
	      	<div class="result-grid-header">
   				<div class="result-grid-tools">
   				    <span class="result-grid-tool" onclick="bPoupDivDisplayPublish('list');">
			      		 ...
			      		 <tbody  class="result-grid-tbody">
			      		 	<c:forEach items="${page.data}" var="list"  varStatus="status"> 
				      		 	<tr>
				      		 		...
				      		 		<td  class="td-css">
				      		 			<input id="isDisplay${list.DISPLAY_ID}" value="${list.isDisplay}" type="hidden"/>
