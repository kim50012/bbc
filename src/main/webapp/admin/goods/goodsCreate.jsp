...
.goods-img {width: 100px; height: 100px; border: 1px solid #e5e5e5; text-align: center; background-color: #f7f7f7; background-image: url("/admin/images/goods/imgUp.png"); background-repeat: no-repeat; background-position: 40px 30px; cursor: pointer;}
.goods-img span{display: inline-block; font-size: 12px; color: #313131; margin-top: 60px;}
.goods-img-show {display: none; width: 100px; height: 100px; border: 1px solid #e5e5e5; text-align: center; background-repeat: no-repeat; background-position: 0px 0px; cursor: pointer; background-size: contain;}
...
<script type="text/javascript">
				...
				$.ajax({
					...
					success : function(data){
						...
						if(recount > 0) {
							...
							$(".liListThree").empty();
							$.each(items, function(i, n){
								liList = "<li onclick=\"selectChange('" + items[i].goodsClassId +"','" + items[i].goodsClassNm +"','3')\"><span>" + items[i].goodsClassNm +"</span></li>";
	...
	function logoUpload(file) {
		...
		if (checkImage(file)) {
	 		...
	 		prevDiv.innerHTML='';
	 		$("#preview").attr("style","background-image:url('/admin/images/goods/imgUp.png')");
			$("#preview").append("<span><ui:i18n key='添加图片'/></span>");
		}
	}
	...
	function save() {
		...
		//pageCurr = 브랜드 아이디를 이것으로 대체하여 사용한다.
...
<body>
	...
	<div class="container">
				...
				<div class="ui-box-body">
					<div class="scroll">
						<ol>
							...
							<li onclick="selectChange('${items.goodsClassId}','${items.goodsClassNm}','1')">
		...
		<div class="form-warp clearfix">			
			...
			<div class="control-warp">
				<div class="form-label">
					<ui:i18n key="品牌"/><a class="label-required"></a> :
				</div>
				<div class="form-control">
						<select id="brandId" class="select2 sw33"   >
				            <option value=""></option>
				            <c:forEach items="${brandList}" var="brandList" varStatus="status">
								<option value="${brandList.BRAND_ID}">${brandList.BRAND_NM}</option>
							</c:forEach>
			         	 </select>	
				</div>
			</div>
