...
// 						query(1,10);
	...
	function query(pageNo, pageSize){
		...
		var url = '/admin/goods/display/presaleListFragment.htm';
...
// 					query(1,10)
	...
	function updateMainGoods(flag,goodsId,displayId){
		if(flag == 1){
			alert('<ui:i18n key="此商品已是主页展示商品"/>');
		}else{
			if(window.confirm('<ui:i18n key="你确定要将这条商品设置为主页展示商品吗"/>？')){
				var param = {
								displayId : displayId
								,goodsId : goodsId
							};
				$.ajax({
					data : param,
					url : '/admin/goods/display/updateMainGoods.htm',  
					success : function(data){
						if(data.result == 'success'){
							alert('<ui:i18n key="主页展示商品设置成功"/>');
							window.location.reload();//调用AdminGoodsSellAction中goodsSell方法进行页面刷新
						}else{
							alert('<ui:i18n key="主页展示商品设置失败"/>，<ui:i18n key="请重新操作"/>。');
						}
					},
					error : function(a, msg, c) {
						alert('error: ' + msg);
