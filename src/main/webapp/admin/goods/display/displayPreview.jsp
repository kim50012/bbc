			...
			<table style="width: 100%;">
				<tr>
					<td style="padding-left: 8px;">
						<span class="detail_headtitle">${goodsDisplayInsert.descTitle }</span>
					</td>
					<td></td>
					<td rowspan="2">
...
<%-- 								onclick="favorites('${detail.FAVORITES_GOODS_FLAG}','${detail.SHOP_ID}','${detail.GOODS_ID}','${detail.DISPLAY_ID}','grid')"> --%>
<%-- 						</c:if> --%>
						<img  style="width: 24px;height: 20px;vertical-align: middle;" alt="" src="/front/skin/main/img/favorites_01.png">
					</td>
				</tr>
				<tr>
					<td style="padding-left: 8px;">
						<span class="detail_subtitle">${goodsDisplayInsert.descDetail }</span>
					</td>
					<td></td>
					<td align="center">
<%-- 						<c:if test="${detail.FAVORITES_GOODS_FLAG=='Y' }"> --%>
				...
				<tr>
					<td colspan="4" style="padding-top: 8px;padding-left: 8px;">
						...
						<div class="detail_bought_count" style="padding-top: 3px;">
							<span class="detail_desc"><c:if test="${empty(goodsDispalyPriceInsert.sumPayQty) or goodsDispalyPriceInsert.sumPayQty == '' }">0</c:if><c:if test="${empty(goodsDispalyPriceInsert.sumPayQty) or goodsDispalyPriceInsert.sumPayQty == '' }">0</c:if>${goodsDispalyPriceInsert.sumPayQty }人付款</span>
						</div>
						<div class="detail_favorite_count" style="padding-top: 3px;">
							<span class="detail_desc""><c:if  test="${empty(goodsDispalyPriceInsert.sumFvortQty) or goodsDispalyPriceInsert.sumFvortQty == ''  }">0</c:if>${goodsDispalyPriceInsert.sumFvortQty }人收藏</span>
						</div>
					</td>
				</tr>
				<tr>
...
<%-- 							</c:choose> --%>
			...
			<ul id="tttt" class="detail_tabSt1 tab">
				...
				<li id="lireview"><a href="#pCont3">评价(<span><c:if test="${empty(totalRecords) or totalRecords == ''  }">0</c:if>${totalRecords}</span>)</a></li>
			</ul>
