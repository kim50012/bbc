...
<c:choose>
	<c:when test="${type eq 1 }">
		...
		$(function(){
			...
			$('#selectedMsg').click(function(){
				...
				if(!!window.parent && !!window.parent.setttingType52Image){
					window.parent.setttingType52Image(array);
				}
			});
	...
	<c:when test="${type eq 3 }"></c:when>
	<c:when test="${type eq 4 }"></c:when>
	<c:when test="${type eq 5 }"></c:when>
	<c:when test="${type eq 6 }"></c:when>
	<c:when test="${type eq 7 }"></c:when>
	<c:when test="${type eq 8 }"></c:when>
	<c:otherwise>
...
<body>
	...
	<form id="searchForm" action="/admin/weixin/material/showImageWindow.htm">
		...
		<input type="hidden"  name="type" id="linkType" value="${linkType }" />
	...
	<div class="popupWarp" >
		<div  class="headerArea">
			<div class="searchArea" style="">
				 <input type="text" id="searchTitle" value="${title }" placeholder="${label.标题}" style="margin-left: 10px;" >
					...
					<c:forEach items="${page.data}" var="item" varStatus="subStatus">
						<li class="img_item js_imageitem" 
						    ...
						    <label class="frm_checkbox_label img_item_bd"  
								...
								data-img-file-id="${item.IMG_FILE_ID }" 
								data-img-file-url="${item.GOODS_LIST_IMG_URL }">
						        <img src="${item.GOODS_LIST_IMG_URL }" alt="${item.PAGE_NM }" class="pic">
						        <span class="lbl_content" title="${item.PAGE_NM }" >${item.PAGE_NM }</span>
