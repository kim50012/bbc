...
<script type="text/javascript">
	$(function(){
		...
		$('.submit-div.confirm').click(function(){
			...
			if(custSysId == ''){
				alert('<ui:i18n key="请选择顾客"/>');
				return;
			}
			custSysId = custSysId.substring(0,custSysId.length - 3);
			nickName = nickName.substring(0,nickName.length - 3);
			parent.window.closeCustDiv(custSysId,nickName);
		});
	});
	//分页操作
	function gets_value(){
		...
		window.location.href = '/admin/msgControl/selectCust.htm?pageCurr=' + pageCurr + '&custNickName='+custNickName;
	}
	...
	function selectByName(){
		var custNickName = $('#custNickName').val();
		window.location.href = '/admin/msgControl/selectCust.htm?custNickName=' + custNickName;
	}
				...
				<c:forEach items="${list}" var="list" varStatus="status">
					<div class="img-one-div">
						<div class="image-show" >
							...
							<div class="img-select" data-custsysid="${list.CUST_SYS_ID }" data-nickname="${list.CUST_NICK_NM }" ></div>
							<img src="/admin/images/sendMsgAll/ico_choice_02.png" class="selected-img-top"/>
						</div>
						<span class="image-name" title="${list.CUST_NICK_NM }">
							${list.CUST_NICK_NM }
						</span>
					</div>
				</c:forEach>
