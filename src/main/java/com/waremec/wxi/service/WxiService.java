...
import com.waremec.wxi.utilities.Utilities;
...
import com.waremec.wxi.thread.WxiSendMsgThread;
...
*/
@Service
public class WxiService extends CommonServiceImpl implements CommonService,ServletContextAware {
...
//		logger.info("===============================postData===============================");
		...
		if (Constants.MsgType.EVENT.equals(MsgType)) {//接收事件推送
			...
			if (Constants.Event.SUBSCRIBE.equals(Event)) {// 关注事件 OR 扫描带参数二维码事件(用户未关注时，进行关注后的事件推送)
				...
				///////////////////////////////////
				//需改善的点， HttpClient发送可能占用一些时间，最好用其他线程处理 -- added by zhengtaifan at 20160216
				if(appInfo.getInterfaced() == 1){//判断是否需要给第三方推送用户信息
 					UserReturn userReturn = WeixinUserUtils.getUserInfoById(
							token, FromUserName);
					List<NameValuePair> params = new ArrayList<NameValuePair>();
					params.add(new BasicNameValuePair("nickname",userReturn.getNickname()));
					params.add(new BasicNameValuePair("sex",userReturn.getSex()));
					params.add(new BasicNameValuePair("openid",userReturn.getOpenid()));
					params.add(new BasicNameValuePair("country",userReturn.getCountry()));
					params.add(new BasicNameValuePair("province",userReturn.getProvince()));
					params.add(new BasicNameValuePair("city",userReturn.getCity()));
					params.add(new BasicNameValuePair("language",userReturn.getLanguage()));
					params.add(new BasicNameValuePair("headImgUrl",userReturn.getHeadimgurl()));
					params.add(new BasicNameValuePair("subscribeTime",DateUtil.toString(DateUtils.secondsToDate(Utilities.toLong(CreateTime)),DateUtil.DEFAULT_DATETIME_FORMAT_SEC)));
					params.add(new BasicNameValuePair("unsubscribeTime",""));
					params.add(new BasicNameValuePair("subscribe",Utilities.toString(userReturn.getSubscribe())));
					String url = appInfo.getIfPushUrl();
					//"https://www.gcbnh.com:454/gchzm/gchzm_fw.asp"; 
					String returns = HttpClientUtils.requestPost(url,params);
 				}
				//需改善的点， HttpClient发送可能占用一些时间，最好用其他线程处理 -- added by zhengtaifan at 20160216 end
				///////////////////////////////////
 				
				
 				logger.info("000000000000000000:User SUBSCRIBE");
  				List<Article>  ars_Msg = wxiDao.dkfAccountByARSKeyword("SUBSCRIBE",appInfo.getShopId());
				Thread msgThread = new Thread(new WxiSendMsgThread(request, weixinService, requestMap,ars_Msg,appInfo,context));
				msgThread.start();
...
//					returnMsg = WeixinResponseMessageUtils.genResponseMsg(request,appInfo,requestMap,weixinMessageDao.getSubscribeMessage(appInfo));
				...
				if(appInfo.getInterfaced() == 1){ // 判断是否给第三方推送用户信息
					WXUser userReturn = weixinUserService.getUserInfoByOpenid(user.getOpenid());
					//WeixinUserUtils.getUserInfoById(token , user.getOpenid());
					List<NameValuePair> params = new ArrayList<NameValuePair>();
					params.add(new BasicNameValuePair("nickname",userReturn.getNickname()));
					params.add(new BasicNameValuePair("sex",userReturn.getSex()));
					params.add(new BasicNameValuePair("openid",userReturn.getOpenid()));
					...
					params.add(new BasicNameValuePair("language",userReturn.getLanguage()));
					params.add(new BasicNameValuePair("headImgUrl",userReturn.getHeadimgurl()));
					params.add(new BasicNameValuePair("subscribeTime",DateUtil.toString(userReturn.getSubscribe_time(),DateUtil.DEFAULT_DATETIME_FORMAT_SEC)));
					params.add(new BasicNameValuePair("unsubscribeTime",DateUtil.toString(DateUtils.secondsToDate(Utilities.toLong(CreateTime)),DateUtil.DEFAULT_DATETIME_FORMAT_SEC)));
					params.add(new BasicNameValuePair("subscribe",Utilities.toString(userReturn.getSubscribe())));
					String url = appInfo.getIfPushUrl();
					//"https://www.gcbnh.com:454/gchzm/gchzm_fw.asp"; 
					String returns = HttpClientUtils.requestPost(url,params);	
...
//				           </xml>
			...
			if (Constants.MsgType.TEXT.equals(MsgType)) {// 接收普通消息 - 文本消息
				...
				insertTextMsg(textMsg);//保存聊天记录
				logger.info("inserted....");
				
				String token = weixinService.getAccessToken(appInfo);
				List<DKFAcountDomain> accountList_DKFKeyword = wxiDao.dkfAccountByDKFKeyword(Content,appInfo.getShopId());//获取多客服关键词坐席列表
 				List<Article> ars_Msg = wxiDao.dkfAccountByARSKeyword(Content,appInfo.getShopId());// 获取ARS关键词 ARS内容
 				String kf_account = "";
				
				if(appInfo.getTransferCustomerService() == 1 && accountList_DKFKeyword.size() > 0){//如果客户发送内容为多客服关键词   转发到多客服绑定该关键词坐席人员
					...
					try {
 						...
 						params.put("shopId", appInfo.getShopId());
 						params.put("shopNm", appInfo.getShopNm());
 						params.put("errorCode", e.getMessage());
 						insert("wxi.wxInterfaceError", params);
 						accountList_online = new ArrayList<OnlineKflist>();
 					}
					if(accountList_online.size() > 0){//判断是否所有坐席 OFFLINE
						System.out.println("转发到多客服绑定该关键词坐席人员");
						for(int i=0;i<accountList_DKFKeyword.size();i++){
							for(int j=0;j<accountList_online.size();j++){
								if(accountList_online.get(j).getKf_account().equals(accountList_DKFKeyword.get(i).getAccount())){//判断该关键词所属坐席在线状态
									kf_account = accountList_DKFKeyword.get(i).getAccount();//获取任意一个坐席账号
									System.out.println("该坐席在线 转发到坐席");
 									break;
			   					}
 							}
						}	
					}else{ //该关键词坐席均不在线 
						System.out.println("绑定关键词坐席不在线 转到master坐席");
		 				List<DKFAcountDomain> accountList_Master = wxiDao.dkfAccountByMaster(appInfo.getShopId(),accountList_DKFKeyword);//获取拥有该关键词全部坐席的坐席组master
   						if(accountList_online.size() > 0){//不为所有坐席都离线
   							if(accountList_Master.size() > 0){//不为拥有该关键词坐席所属坐席组master都离线
   								for(int i=0;i<accountList_Master.size();i++){
									for(int j=0;j<accountList_online.size();j++){
										if(accountList_Master.get(i).getAccount().equals(accountList_online.get(j).getKf_account())){//拥有该关键词坐席所属坐席组master在线
											kf_account = accountList_Master.get(i).getAccount();//取任意一个在线master
			   								System.out.println(" master master在线 转到master");
		 									break;
										}
									}
								}
   							}else{//关键词坐席不在线 转发到 master master不在线 转到任何在线坐席
    							System.out.println("关键词坐席不在线 转发到 master master不在线 转到任何在线坐席");
   								for(int j=0;j<accountList_online.size();j++){
									kf_account = accountList_online.get(j).getKf_account();
 									break;
   								}
 							} 
						}else{
					...
					if(kf_account == null){//所有坐席不在线 推送购物向导,并把消息扔到多客服系统
						System.out.println("所有坐席不在线 推送客服不在线向导,并把消息扔到多客服系统");
						if(ars_Msg.size() <= 0){
							ars_Msg = wxiDao.dkfAccountByARSKeyword("DKFOFFLINE",appInfo.getShopId());
							if(ars_Msg.size() > 0){
								Thread msgThread = new Thread(new WxiSendMsgThread(request, weixinService, requestMap,ars_Msg,appInfo,context));
								msgThread.start();
							}
 						}
 						//转发给多客服系统 有坐席登录后马上收到消息
						...
						System.out.println("转发到绑定关键词坐席");
						System.out.println(kf_account);
						String responseMsg =  XmlTemplete.transferCustomerService(FromUserName, ToUserName, time, kf_account);
						logger.info("transfer_customer_service2===" + responseMsg);
						System.out.println(responseMsg);
						return pc.encryptMsg(responseMsg, time, nonce);
					}
				}else if(ars_Msg.size() > 0){ //回复该关键词设定ARS
					System.out.println("回复该关键词设定ARS");
					Thread msgThread = new Thread(new WxiSendMsgThread(request, weixinService, requestMap,ars_Msg,appInfo,context));
					msgThread.start();
					System.out.println(ars_Msg.get(0).getTitle().contains("人工客服"));
					if(ars_Msg.get(0).getTitle().contains("正在转接人工客服，请稍后…")){
						//用户发送菜单选项 对应回复内容包含"人工客服"字样 消息转发至多客服
						String responseMsg =  XmlTemplete.transferCustomerService(FromUserName, ToUserName, time);
						logger.info("将消息转发至多客服");
						logger.info("transfer_customer_service1===" + responseMsg);
						System.out.println(responseMsg);
 					...
 					System.out.println("推送购物向导。");
					if(ars_Msg.size() <= 0){
						ars_Msg = wxiDao.dkfAccountByARSKeyword("ARSMENU",appInfo.getShopId());
						if(ars_Msg.size() > 0){
							Thread msgThread = new Thread(new WxiSendMsgThread(request, weixinService, requestMap,ars_Msg,appInfo,context));
							msgThread.start();	
						}
