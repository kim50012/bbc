...
import com.waremec.weixin.domain.customservice.GetRecordReturn;
import com.waremec.weixin.domain.customservice.GetSessionListReturn;
import com.waremec.weixin.domain.customservice.GetWaitCaseReturn;
import com.waremec.weixin.domain.customservice.GetkflistReturn;
import com.waremec.weixin.domain.customservice.Record;
import com.waremec.weixin.domain.msg.Article;
import com.waremec.weixin.exception.WeixinErrorException;

/**
 * @Package com.waremec.weixin.utils
 * @FileName WeixinMessageUtils
 * @Description 消息(客服,多客服)接口工具类
 * @author taebem
 * @date 2015年5月7日 下午3:56:47
 * 
 */
public class WeixinMessageUtils extends WeixinBaseUtils {
	...
	public static void main(String[] args) throws ParseException {
		// TODO Auto-generated method stub
 		...
 		String token = TokenUtils.getToken(15);
		logger.info("token==" + token);
		
		logger.info("********************************************************");
 		 /* sendTextMsg(token, "oGKUAwVtiFS45XtMgws3UubhGlyA", "您好！非常感谢您添加新世界免税店的微信号。"+
			"我们在此为大家准备了诸多优惠，"+
			"请与新世界免税店一起尽享愉悦旅程！"+
			""+
			"[1] 购物导购漫画"+
 		  ...
 		  sendTextMsg(token, "oBjf0smytvgTeKrhN5k6WBtkWt_4", "您好！非常感谢您添加韩国绿十字健康管理微信号。"); 
		// logger.info("********************************************************");
		// sendMusicMsg(token, "o1yuEtw1PN5sc6W6Bxd7Q4hty2hY", "title",
		// "description", "musicurl", "hqmusicurl", "thumb_media_id");
		// logger.info("********************************************************");
//		String domain = "http://www.hanzhimeng.com";
//		 List<Article> list = new ArrayList<Article>();
//		 list.add(new Article("지금 바로 이벤트 참여하세요", "상품이 1등 굴삭기, 2등 휠로더 !!!!!!", domain + "/uploads/8/2015/06/19/8_material_2015_06_19_14_54_44_56500.png",domain +  "/front/eventRoulette/eventRoulette.htm?gameId=6"));
//		 list.add(new Article("title", "description", "picurl", "url"));
//		 list.add(new Article("title", "description", "picurl", "url"));
		 ////
		 
//		 sendNewsMsg(token, "o1yuEtw1PN5sc6W6Bxd7Q4hty2hY", list);
		// logger.info("********************************************************");
		logger.info("********************************************************");
//
//
////		 getkflist(token);
////		 getonlinekflist(token);
//		// getDkfRecord(token);
//
		 addKfAccount(token,"test2@Waremec","test1","123456");
//		// updateKfAccount(token,"taebem@Waremec","taebem","112233");
//		// delKfAccount(token,"test1@Waremec");
//		 /////////////////////////////////////
//		 SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss");
//		 String sdate = "20151008";
//		 Date starttime = sdf.parse(sdate + "000000");
//		 Date endtime =   sdf.parse(sdate + "235959");
//		
//		 GetRecordReturn getRecordReturn = getDkfRecord(token, starttime,endtime, 4, null);
//		 List<Record> recordList =  getRecordReturn.getRecordlist();
//		 logger.info("recordList size====" + recordList.size());
//		 for (Record item : recordList) {
//			logger.info(item);
//		}
		 /////////////////////////////////////////

		// createKfSession(token, "o1yuEtw1PN5sc6W6Bxd7Q4hty2hY",
		// "taebem@Waremec", "nihao");
		//
		// getCustSessionStatus(token, "o1yuEtw1PN5sc6W6Bxd7Q4hty2hY");

		// closeKfSession(token, "o1yuEtw1PN5sc6W6Bxd7Q4hty2hY",
		// "taebem@Waremec", "zaijian");
		//
		// getCustSessionStatus(token, "o1yuEtw1PN5sc6W6Bxd7Q4hty2hY");

		// getKfSessionList(token,"taebem@Waremec");

		// getWaitCase(token);

		// getCurrentAutoreplyInfo(token);

		// sendTemplateMsg(token,"ZxRqBY-Ijm-ItQZK2R-RG1SrnnUvYM_BIrrWXNfwD2o",OPENID);
	}
...
//		JSONObject jsonObject = JSONObject.fromObject(postData);
	 ...
	 *            是 缩略图的媒体ID
	 */
	public static void sendMusicMsg(String access_token, String touser, String title, String description, String musicurl, String hqmusicurl,
	 ...
	 */
	/*
	 * public static void sendCard(String access_token, String touser, String
	 * card_id, String card_ext){ String url =
	 * getInterfaceUrl(MESSAGE_CUSTOM_SEND_URL, access_token);
	 * 
	 * String postData = "{\"touser\":\"" + touser +
	 * "\",\"wxcard\":{\"card_id\":\"" + card_id + "\", \"card_ext\":\"" +
	 * card_ext + "\"}}"; JSONObject jsonObject =
	 * JSONObject.fromObject(postData);
	 * 
	 * String result = HttpClientUtils.requestPost(url,jsonObject);
	 * 
	 * WeixinReturn weixinReturn = new Gson().fromJson(result,
	 * WeixinReturn.class); if(weixinReturn.isError()){ throw new
	 * WeixinErrorException(weixinReturn); } }
	 */
	 ...
	 *            是 客户openid
	 * @param kf_account
	 *            是 完整客服账号，格式为：账号前缀@公众号微信号
	 * @param text
	 *            否 附加信息，文本会展示在客服人员的多客服客户端
	 */
	 ...
	 * @param access_token
	 * @param kf_account
	 *            是 完整客服账号，格式为：账号前缀@公众号微信号，账号前缀最多10个字符，必须是英文或者数字字符
	 * @return
	 */
