...
//		 logger.info("****************************");
//		  logger.info("userInfoUrl====" + getSnsapiUserinfoUrl(appInfo,"","ssssss"));
//		 logger.info("****************************");
//		  String menuUrlOld = getMenuIinkUrl(appInfo, 1000001);
//		  String menuUrlNew = getMenuIinkUrlNew(appInfo, 1000001);
//		 
//		  logger.info("menuUrlOld======" + menuUrlOld);
//		  logger.info("menuUrlNew======" + menuUrlNew);
//		  logger.info("****************************");
//		  String adviceUrlNew = getAdviceLinkUrl(appInfo, "/front/main/home.htm");
//		  logger.info("adviceUrlNew====" + adviceUrlNew);
//		  logger.info("****************************");
//		  String letterUrlOld = getLetterLinkUrl(appInfo, 1000001);
//		  String letterUrlNew = getLetterLinkUrl(appInfo, 1000001,"o1yuEtw1PN5sc6W6Bxd7Q4hty2hY");
//		  
//		  logger.info("letterUrlOld====" + letterUrlOld);
//		  logger.info("letterUrlNew====" + letterUrlNew);
//		  logger.info("****************************");
//		  String shareUrlOld = getShareLinkUrl(appInfo, "/front/main/home.htm");
//		  String shareUrlNew = getShareLinkUrl(appInfo, "/front/main/home.htm","o1yuEtw1PN5sc6W6Bxd7Q4hty2hY");
//		  logger.info("shareUrlOld====" + shareUrlOld);
//		  logger.info("shareUrlNew====" + shareUrlNew);
//		  logger.info("****************************");
//		  String ssoUrl = getSSOIinkUrl(appInfo, 900000001);
//		  logger.info("ssoUrl====" + ssoUrl);
//		  logger.info("****************************");
		  logger.info("****************************");
		  String ssoUrl = getSSO2IinkUrl(appInfo, "http://global.hyundaihmall.com/GL/pda/itemPtc.do?sectId=1529684&slitmCd=2011167607&GoogleAPI=cn");
		  logger.info("ssoUrl====" + ssoUrl);
		  logger.info("****************************");
	}
...
//		logger.info("Full URL Length = " + sb.toString().length());
	...
	public static String getOauth2SnsapiBaseUrlNormal(AppInfo appInfo, String redirectUrl, String linkType) {

		StringBuffer sb = new StringBuffer();

		String encodedUrl;
		try {
			encodedUrl = URLEncoder.encode(redirectUrl, UTF_8);
		} catch (UnsupportedEncodingException e) {
			logger.error(e.getMessage());
			throw new WeixinErrorException(e.getMessage());
		}
	...
	public static String getMenuIinkUrlNew(AppInfo appInfo, Integer menuId) {
		try {
...
//				logger.info("encodedRedirectUrl=" + encodedRedirectUrl);
	...
	public static String getEventShareLinkUrl(AppInfo appInfo, String targetUrl, String openid, Integer eventId,Integer jlcount){
		try {
			//判断链接后加的信息数量参数
			if (StringUtils.isNotEmpty(targetUrl)) {
				if (targetUrl.indexOf("shopId=") < 0) {
...
//				logger.info("encodedRedirectUrl=" + encodedRedirectUrl);
	...
	public static String getShareLinkUrl(AppInfo appInfo, String shareUrl) throws Exception {
		...
		StringBuffer sb = new StringBuffer();
		sb.append("https://open.weixin.qq.com/connect/oauth2/authorize").append("?appid=").append(appInfo.getAppId()).append("&redirect_uri=")
				.append(encodedUrl).append("&response_type=").append("code").append("&scope=").append("snsapi_base").append("&state=").append("shareLink")
				.append("#wechat_redirect");
...
//		logger.info("Encoded URL = " + encodedUrl);
		...
		StringBuffer sb = new StringBuffer();
		sb.append("https://open.weixin.qq.com/connect/oauth2/authorize").append("?appid=").append(appInfo.getAppId()).append("&redirect_uri=")
				.append(encodedUrl).append("&response_type=").append("code").append("&scope=").append("snsapi_base").append("&state=").append("0")
				.append("#wechat_redirect");
...
//		logger.info("Full URL Length = " + sb.toString().length());
	...
	public static WebAccessToken getWebAccessToken(AppInfo appInfo, String code) {
		...
		String url = SNS_OAUTH2_ACCESS_TOKEN_URL.replaceAll("APPID", appInfo.getAppId()).replaceAll("SECRET", appInfo.getAppSecret()).replaceAll("CODE", code);
	...
	public static WebAccessToken refreshToken(AppInfo appInfo, String refresh_token) {
		...
		String url = SNS_OAUTH2_REFRESH_TOKEN_URL.replaceAll("APPID", appInfo.getAppId()).replaceAll("REFRESH_TOKEN", refresh_token);
