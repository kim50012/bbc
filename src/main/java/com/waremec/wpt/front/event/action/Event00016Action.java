/**   
* @Title: Event00016Action.java 
* @Package com.waremec.wpt.front.event.action 
* @Description:  
* @author taebem
* @date 2015年9月11日 下午1:04:47 
* @version V1.0   
*/
package com.waremec.wpt.front.event.action;
...
/** 
 * @Package com.waremec.wpt.front.event.action 
 * @FileName Event00016Action
 * @Description  
 * @author taebem 
 * @date 2015年9月11日 下午1:04:47 
 *  
 */
@Controller("event00016Action")
@Scope(ScopeType.prototype)
public class Event00016Action extends EventBaseAction {
	...
	/*	if(5 > 1){
		...
		*/
		...
		if(StringUtils.isBlank(fromOpenid)){
			...
			Map<String, Object> resultMap = eventService.select("Event.EVENT_00016_POINT_CHECK",parmsMap);
			
			String isFirst = (String)resultMap.get("SUCCESS");
			
			if("Y".equals(isFirst) && sessionMember != null){
				Map<String, Object> parmsMap1 = new HashMap<String, Object>();
				parmsMap1.put("EVENT_ID", eventId);
				parmsMap1.put("OPENID", getSessionOpenid());
				parmsMap1.put("POINT_TYPE", EVENT_TYPE_IS_FIRST);
				parmsMap1.put("POINT", 300);
				
				eventService.select("Event.EVENT_00016_POINT_INSERT",parmsMap1);
				/*jsSdkWithEventSttingNotOpenid();
				eventInit();
				jsSdkStting();
				return "participate";*/
		...
		if(StringUtils.isNotEmpty(getFromOpenid())){
			...
			Map<String, Object> userSummary = eventService.select("Event.EVENT_00016_POINT_GET_SUM",parmsMap);
			...
			parmsMap.put("EVENT_ID", eventId);
			parmsMap.put("OPENID", getFromOpenid());
			List<Map<String, Object>> friendsList = eventService.selectList("Event.EVENT_00016_POINT_FRIENDS_LIST",parmsMap);
			...
			Map<String, Object> checkItem = eventService.select("Event.EVENT_00016_POINT_CHECK",parmsMap);
	...
	public String participated() {
		...
		if(StringUtils.isBlank(fromOpenid)){
			...
			eventService.select("Event.EVENT_00016_POINT_INSERT",parmsMap);
...
//			  , OBTAIN_CNT = (SELECT COUNT(*) FROM EVENT_00016_4500)
//			  ,  SELF_CNT = (SELECT COUNT(*) FROM EVENT_00016_4500 WHERE OPENID = @FROM_OPENID)
//		
		...
		if(selfCnt > 0){
			...
			String msg4500 = "<div class='result4500' ><img alt='' src='/front/event/00016/images/participated/img_congratulation.png'  ></div> "
					+"<div class='result4500Close'  ><img alt=''  onclick='closeResult()'  src='/front/event/00016/images/participated/btn_4500_close.png'  ></div>";
		...
		if(selfCnt < 1){
			if(obtainCnt >= 5){//已经5个人中奖不允许其他人中奖
				...
				}else if(totalPoint >= 2000 && totalPoint < 3500 ){
					angle = new Angle(1000, rand.nextInt(35) + 320);
				}else{
					angle = getAngle();
				}
			}else{
//				Integer lastNumber =Integer.valueOf(totalPoint.toString().substring(totalPoint.toString().length()-1));
//				if(lastNumber == 6){
//					angle = getAngle1() ;
//				}else{
					angle = getAngle() ; 
...
//		Map<String, Object> resultMap = eventService.select("Event.EVENT_00016_POINT_INSERT",parmsMap);
		eventService.select("Event.EVENT_00016_POINT_INSERT",parmsMap);
...
public String everybodyLoves(){
	jsSdkStting();
	return "everybodyLoves";
}
...
public String product_list_shiyan(){
	jsSdkStting();
	return "product_list_shiyan";
}

public String product_list_winter(){
	jsSdkStting();
	return "product_list_winter";
}
